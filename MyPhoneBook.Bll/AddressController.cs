...
namespace MyPhoneBook.Controllers
{
    ...
    [ApiController]
    public class AddressesController : PhoneBookControllerBase
    {
        ...
        {
            var contact = await _contactService.GetContactById(addressRequest.ContactId);
            if (contact != null)
            {
                var addressInfo = new AddressModel()
                {
                    ContactId = addressRequest.ContactId,
                    City = addressRequest.City,                 // ete chunenam contact id chem karum contactis address@ stanam
        ...
        {
            var addressRecord =await _addressService.GetAddressById(id);
        ...
        {             
            ...
            if (result)
            {
                return new JsonResult($"successfully deleted address with id {id}");
            }
